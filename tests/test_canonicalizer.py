...
@pytest.mark.parametrize(
    ("symmetry_name", "first_component", "second_component", "first_role", "second_role"),
    [
        ("L0.qk", "query.weight", "key.weight", "query", "key"),
        ("L0.ov", "value.weight", "output.dense.weight", "value", "output"),
    ],
)
def test_cascading_template_canonicalizer_save_load_roundtrip_for_dual_attention_templates(
    tmp_path,
    symmetry_name,
    first_component,
    second_component,
    ...
    symmeters = Symmeters([symmetry_name])
    symmeters.add_component(
        symmetry_name,
        first_component,
        torch.eye(2),
        axes=("rows", symmetry_name),
        kind="weight",
        layout="identity",
        parameter_keys=first_component,
    )
    symmeters.add_component(
        symmetry_name,
        second_component,
        torch.diag(torch.tensor([6.0, 12.0])),
        axes=("rows", symmetry_name),
        kind="weight",
        layout="identity",
        parameter_keys=second_component,
    )
    ...
    canonicalizer = CascadingTemplateCanonicalizer(
        ...
        templates={
            symmetry_name: {
                first_role: torch.diag(torch.tensor([2.0, 3.0])),
                second_role: torch.diag(torch.tensor([3.0, 4.0])),
            },
        },
    )
    path = tmp_path / f"cascade-{symmetry_name.replace('.', '-')}.pt"
    canonicalizer.save(str(path))
    ...
    torch.testing.assert_close(
        canonicalized.tensor(symmetry_name, first_component),
        torch.diag(torch.tensor([2.0, 3.0])),
    )
    torch.testing.assert_close(
        canonicalized.tensor(symmetry_name, second_component),
